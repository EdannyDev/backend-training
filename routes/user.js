...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    // Generar token
    const token = jwt.sign({ user: { id: newUser._id } }, process.env.JWT_SECRET, { expiresIn: '30s' });
...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    // Generar token
    const token = jwt.sign({ user: { id: user._id, role: user.role } }, process.env.JWT_SECRET, { expiresIn: '30s' });
